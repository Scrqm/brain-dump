...
/**
 Merge array `brainDumps` into a single `BrainDumpStruct` representing maximum values and combined `symptoms`, `activities` and `social` values (via logical AND).
 */
@inline(__always) func mergeBrainDumps(brainDumps: [BrainDumpStruct]) -> BrainDumpStruct? {
    if brainDumps.count == 0 {
        return nil
    }

    var collapsed = BrainDumpStruct()

    for brainDump in brainDumps {
        collapsed.timestamp = brainDump.timestamp
        collapsed.elevation = max(collapsed.elevation,
                                  brainDump.elevation)
        collapsed.depression = max(collapsed.depression,
                                   brainDump.depression)
        collapsed.anxiety = max(collapsed.anxiety,
                                brainDump.anxiety)
        collapsed.irritability = max(collapsed.irritability,
                                     brainDump.irritability)

        for i in 0 ..< symptomList.count {
            collapsed.symptoms[i] = collapsed.symptoms[i] || brainDump.symptoms[i]
        }

        for i in 0 ..< activityList.count {
            collapsed.activities[i] = collapsed.activities[i] || brainDump.activities[i]
        }

        for i in 0 ..< socialList.count {
            collapsed.social[i] = collapsed.social[i] || brainDump.social[i]
        }
    }
