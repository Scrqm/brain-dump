...
/**
 Get date range of `brainDumps` in days.
 */
@inline(__always) func dateRange(brainDumps: [BrainDumpStruct]) -> Int {
    let first = getFirstDate(brainDumps: brainDumps)
 ...
 */
@inline(__always) func getTimescale(timescale: Int, brainDumps: [BrainDumpStruct]) -> Int {
    if timescale == TimeScaleEnum.all.rawValue {
        return max(dateRange(brainDumps: brainDumps), TimeScaleEnum.month.rawValue)
    } else {
        return timescale
    }
}
