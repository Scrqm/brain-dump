...
func generatePDF(data: DataStoreClass, timescale: Int, blackAndWhite: Bool) -> URL? {
    ...
    document.info.author = "BrainDump"
    document.info.title = "BrainDump Report"
    ...
    do {
        try url = generator.generateURL(filename: "BrainDump report.pdf")
    } catch {
        // print("PDF error")
    }
...
func generateTitleHeaderFooterContent(document: PDFDocument, data: DataStoreClass) {
    ...
    // Header
    var headerStr = NSLocalizedString("BrainDump", comment: "BrainDump report")
    ...
    // Title
    let attributedTitle = NSAttributedString(string: NSLocalizedString("BrainDump", comment: "BrainDump report") + "\n", attributes: titleAttributes)
...
func generateNotesContent(document: PDFDocument, data: DataStoreClass, timescale: Int) {
    ...
    for brainDump in data.brainDumps {
        if brainDump.notes != "" && brainDump.snapType != .quote && brainDump.snapType != .custom {
            let attributedNotes = NSAttributedString(string: "\n" + brainDump.timestamp.dateTimeString() + "\n" + brainDump.notes + "\n", attributes: notesAttributes)
