...
struct TallyView: View {
    ...
    var body: some View {
        let windowBrainDumps = getBrainDumpsByDateWindow(brainDumps: data.brainDumps, date: Date(), windowStart: -timescale, windowEnd: 0)
        let (symptomOccurrences, activityOccurrences, socialOccurrences) = countAllOccurrences(brainDumps: windowBrainDumps, settings: data.settings)
        let hashtagOccurrences: [Int] = countHashtagOccurrences(hashtags: data.hashtagList, brainDumps: windowBrainDumps)
        let eventsList = getEventsList(brainDumps: data.brainDumps, window: timescale)
