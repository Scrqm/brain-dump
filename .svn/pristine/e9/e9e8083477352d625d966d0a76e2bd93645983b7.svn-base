...
/**
 Count the total occurrences of symptoms, activites & social within the array `brainDumps`.
 */
@inline(__always) func countAllOccurrences(brainDumps: [BrainDumpStruct], settings: SettingsStruct) -> ([Int], [Int], [Int]) {
    ...
    for brainDump in brainDumps {
        for i in 0 ..< symptomList.count {
            if brainDump.symptoms[i] && settings.symptomVisibility[i] {
                symptomCount[i] += 1
            }
        }

        for i in 0 ..< activityList.count {
            if brainDump.activities[i] && settings.activityVisibility[i] {
                activityCount[i] += 1
            }
        }

        for i in 0 ..< socialList.count {
            if brainDump.social[i] && settings.socialVisibility[i] {
                socialCount[i] += 1
            }
        }
    }
