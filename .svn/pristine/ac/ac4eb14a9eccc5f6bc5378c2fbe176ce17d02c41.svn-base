...
struct DataStoreStruct: Identifiable, Codable, Hashable {
    ...
    var brainDumps: [BrainDumpStruct] = makeIntroSnap()
...
final class DataStoreClass: Identifiable, ObservableObject {
    ...
    @Published var brainDumps: [BrainDumpStruct] = makeIntroSnap()
    ...
    @Published var brainDumpCount: Int = 0
    @Published var firstDate: String = Date().dateString()
    var sequencedBrainDumps: [[BrainDumpStruct]] = []
    var flattenedSequencedBrainDumps: [BrainDumpStruct?] = []
    var healthSnaps: [HealthSnapStruct] = []
    ...
    init(shared: Bool = false, process: Bool = false) {
        ...
        do {
            if shared == false {
                ...
                self.brainDumps = retrieved.brainDumps
                self.healthSnaps = retrieved.healthSnaps
                self.processedData = retrieved.processedData
            } else {
                let retrieved = try Disk.retrieve(
                    "data.json",
                    from: .sharedContainer(appGroupName: "group.BrainDump"),
                    as: DataStoreStruct.self)
    ...
    func processEvents() async -> Bool {
        let eventsListUI = getEventsList(brainDumps: self.brainDumps)
    ...
    func processHashtags() async -> Bool {
        let brainDumps = self.brainDumps
        let hashtagListUI = getHashtags(brainDumps: brainDumps)
        var hashtagButterflies: [ButterflyEntryStruct] = []
        
        for i in 0 ..< hashtagListUI.count {
            let dates = getDatesForHashtag(hashtag: hashtagListUI[i], brainDumps: brainDumps)
    ...
    func processAverages() async -> Bool {
        ...
        let allTimescale = getTimescale(timescale: TimeScaleEnum.all.rawValue, brainDumps: brainDumps)
        
        averages.flatAll = averageBrainDump(timescale: allTimescale, brainDumps: brainDumps, flatten: true)
        averages.allAll = averageBrainDump(timescale: allTimescale, brainDumps: brainDumps, flatten: false)
        averages.flatYear = averageBrainDump(timescale: TimeScaleEnum.year.rawValue, brainDumps: brainDumps, flatten: true)
        averages.allYear = averageBrainDump(timescale: TimeScaleEnum.year.rawValue, brainDumps: brainDumps, flatten: false)
        averages.flatMonth = averageBrainDump(timescale: TimeScaleEnum.month.rawValue, brainDumps: brainDumps, flatten: true)
        averages.allMonth = averageBrainDump(timescale: TimeScaleEnum.month.rawValue, brainDumps: brainDumps, flatten: false)
        averages.flatThreeMonths = averageBrainDump(timescale: TimeScaleEnum.threeMonths.rawValue, brainDumps: brainDumps, flatten: true)
        averages.allThreeMonths = averageBrainDump(timescale: TimeScaleEnum.threeMonths.rawValue, brainDumps: brainDumps, flatten: false)
        averages.flatSixMonths = averageBrainDump(timescale: TimeScaleEnum.sixMonths.rawValue, brainDumps: brainDumps, flatten: true)
        averages.allSixMonths = averageBrainDump(timescale: TimeScaleEnum.sixMonths.rawValue, brainDumps: brainDumps, flatten: false)
        
        volatilities.flatAll = averageVolatilityBrainDump(timescale: allTimescale, brainDumps: brainDumps)
        volatilities.allAll = averageVolatilityBrainDump(timescale: allTimescale, brainDumps: brainDumps)
        volatilities.flatYear = averageVolatilityBrainDump(timescale: TimeScaleEnum.year.rawValue, brainDumps: brainDumps)
        volatilities.allYear = averageVolatilityBrainDump(timescale: TimeScaleEnum.year.rawValue, brainDumps: brainDumps)
        volatilities.flatMonth = averageVolatilityBrainDump(timescale: TimeScaleEnum.month.rawValue, brainDumps: brainDumps)
        volatilities.allMonth = averageVolatilityBrainDump(timescale: TimeScaleEnum.month.rawValue, brainDumps: brainDumps)
        volatilities.flatThreeMonths = averageVolatilityBrainDump(timescale: TimeScaleEnum.threeMonths.rawValue, brainDumps: brainDumps)
        volatilities.allThreeMonths = averageVolatilityBrainDump(timescale: TimeScaleEnum.threeMonths.rawValue, brainDumps: brainDumps)
        volatilities.flatSixMonths = averageVolatilityBrainDump(timescale: TimeScaleEnum.sixMonths.rawValue, brainDumps: brainDumps)
        volatilities.allSixMonths = averageVolatilityBrainDump(timescale: TimeScaleEnum.sixMonths.rawValue, brainDumps: brainDumps)
    ...
    /**
     Process BrainDump stats
     */
    func processStats() async -> Bool {
        let brainDumpCountUI = countBrainDumps(brainDumps: self.brainDumps)
        let firstDateUI = getFirstDate(brainDumps: self.brainDumps).dateString()
        
        DispatchQueue.main.async {
            self.brainDumpCount = brainDumpCountUI
    ...
    func process() async {
        // Sequence BrainDumps
        self.sequencedBrainDumps = sequenceBrainDumps(brainDumps: self.brainDumps)
        self.flattenedSequencedBrainDumps = flattenSequence(sequence: self.sequencedBrainDumps)
    ...
    @inline(__always) func startProcessing(priority: TaskPriority = .high) {
        self.stopProcessing()
        self.save()
        let sorted = sortByDate(brainDumps: self.brainDumps)
        
        DispatchQueue.main.async {
            self.brainDumps = sorted
    ...
    @inline(__always) func save() {
        ...
        do {
            try Disk.save(self.toStruct(),
                          to: .sharedContainer(appGroupName: "group.BrainDump"),
                          as: "data.json")
        } catch {
        }
    ...
    @inline(__always) func toStruct() -> DataStoreStruct {
        ...
        dataStore.brainDumps = self.brainDumps
