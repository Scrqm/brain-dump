 ...
 */
@inline(__always) func getCorrelation(brainDumps: [BrainDumpStruct], healthSnaps: [HealthSnapStruct], type: HealthTypeEnum) -> [CGFloat?] {
    var samples: [CGFloat] = []
    ...
    while date >= earliest {
        ...
        let theseBrainDumps = getBrainDumpsByDate(brainDumps: brainDumps, date: date.addDays(days: dayOffset), flatten: true)

        if theseHealthSnaps.count != 0 && theseBrainDumps.count != 0 {
            switch type {
            case .weight:
                if theseHealthSnaps[0].weight != nil {
                    samples.append(theseHealthSnaps[0].weight!)
                    elevationSamples.append(theseBrainDumps[0].elevation)
                    depressionSamples.append(theseBrainDumps[0].depression)
                    anxietySamples.append(theseBrainDumps[0].anxiety)
                    irritabilitySamples.append(theseBrainDumps[0].irritability)
                }
            case .distance:
