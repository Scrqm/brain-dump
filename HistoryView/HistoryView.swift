    ...
    var body: some View {
        NavigationView {
            ScrollViewReader { scrollView in
                ScrollView {
                    LazyVStack {
                        ForEach(data.brainDumps, id: \.id) { brainDump in
                            HistoryItemView(brainDump: brainDump, filter: $filter, searchText: $searchText)
                        }
                        Spacer()
                    }
                }
                ...
                .onTapGesture(count: 2) {
                    withAnimation(.easeOut(duration: 1.0)) {
                        scrollView.scrollTo(data.brainDumps[0].id, anchor: .bottom)
                    }
                }
                .toolbar {
                    ToolbarItem(placement: ToolbarItemPlacement.navigationBarTrailing) {
                        Menu() {
                            ...
                            Button(action: {
                                filter = .mood
                                searchText = ""
                                searchPrompt = "Search BrainDumps"
                            }) {
                                Label("Search_BrainDumps", systemImage: "brain.head.profile")
                                    .font(.subheadline)
                            }
                            Button(action: {
                                filter = .event
